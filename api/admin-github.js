 ...
 *
 * Phase20で「管理者モードがアプリ内(koufuku-ai.html)へ完全移行した」
 * 理由でいったん廃止したファイルだが、Phase62(Media Library)で
 * 「管理画面で公開したBGM/壁紙/テーマを全ユーザーへ自動配信する」
 * ために復活させた。役割・設計方針・エンドポイント仕様はPhase20廃止前
 * から一切変更していない(単に消えていたファイルを元の設計のまま
 * 戻し、ASSET_DIRSにMedia Library用の2ディレクトリを追加しただけ)。
 *
 ...
 * 新しいCMSモジュールを追加するときは、この関数のコードを変更せず、
 * data/cms/ 以下に新しいJSONファイルを1つ増やすだけでよい
 * (Media Libraryのdata/cms/mediaLibrary.jsonも、この既存の仕組みが
 * 元々サポートしていた汎用JSONパスにそのまま乗るだけで済んだ)。
 *
 * koufuku-ai.html内のアプリ内管理者モード(IndexedDB/CmsStoreベース、
 * Phase20以降の主要な編集手段)とは別物で、この関数は「公開する」操作
 * (Media Libraryタブの「GitHubへ公開」ボタン)からのみ呼ばれる。通常の
 * 編集(下書き保存)は引き続きアプリ内IndexedDBで完結し、この関数にも
 * GitHubにも一切触れない。
 *
 * 保存の仕組み:
 *   koufuku-ai.html(管理者モード「GitHubへ公開」) → (この関数)
 *     → GitHub Contents API でコミット
 *     → 既存の .github/workflows/deploy-mvp-pages.yml が
 *       (paths: img/koufuku/**, audio/koufuku/**, data/cms/** を含むため、
 *       このファイルのワークフロー自体は今回一切変更していない)
 *       自動的にGitHub Pagesを再デプロイ → 数十秒〜1分程度でホーム画面に反映
 *
 * 必要な環境変数 (Vercel Project Settings > Environment Variables):
 *   ADMIN_PASSWORD      (必須) 公開操作用のパスワード。koufuku-ai.htmlの
 *                        Media Libraryタブはこれをそのまま Authorization:
 *                        Bearer ヘッダーで毎リクエスト送る(セッションを
 *                        持たないシンプルな方式)。アプリ内管理者モードの
 *                        ログインパスワード(IndexedDBローカル)とは別物。
...
// 1行足すだけで済むようにしてある。
// Phase62: img/koufuku/room/media/wallpaper と audio/koufuku/media/bgm
// を追加。どちらも既存のdeploy-mvp-pages.ymlが既に再帰コピー対象として
// いるディレクトリ(img/koufuku/room/以下、audio/koufuku/以下)の内側
// なので、ワークフロー自体は一切変更していない。 =====
const ASSET_DIRS = {
  "img/koufuku/room": { exts: [".webp", ".png", ".jpg", ".jpeg", ".gif", ".svg"] },
  "audio/koufuku": { exts: [".mp3", ".ogg", ".wav", ".m4a"] },
  "img/koufuku/room/media/wallpaper": { exts: [".webp", ".png", ".jpg", ".jpeg", ".gif", ".svg"] },
  "audio/koufuku/media/bgm": { exts: [".mp3", ".ogg", ".wav", ".m4a"] },
};
...
// ディレクトリ以外へは、管理画面側が何を送ってきても書き込めない。
...
// か、data/cms/ 配下の *.json (新モジュール用、拡張し放題。Phase62の
// mediaLibrary.jsonもここに乗る) のみ許可する。
...
// 該当コミットのものを探して確認する(保存後、管理画面側が「反映されました」
