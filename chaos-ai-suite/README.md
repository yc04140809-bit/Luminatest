...
- **宣伝パック生成**（`POST /api/note/promo`・担当はミライ）: Threads10本（共感型/気づき型/体験談型/ノウハウ型/実は型/失敗談型/続きはnote型の切り口を分散）・X5本（140字以内）・Instagramキャプション3本・短い告知文3本・記事紹介文・プロフィール誘導文・販売note用CTA・無料note用CTA・**サムネイル案3案**（サムネに載せる短いタイトル・キャッチコピー・デザイン構成・配色を、スマホのCanva等でそのまま再現できる言葉で提案。3案は文字メイン/数字訴求/ビフォーアフター等の方向性を分散）を一括生成。本文の繰り返し禁止・誇大表現禁止をプロンプトで強制。各投稿・各案に個別コピーボタン。結果は下書きに保存されリロード後も残る。使用時のみ+1回のAI呼び出し（サムネ案追加後も呼び出し回数は増えていない）。
...
ココナラ・SNS運用・note編集などの依頼案件を、依頼文の整理から成果物・納品文の作成まで一画面で管理する。案件データはすべてフロント側の`localStorage`（`chaos-ai-suite:cases`）に保存され、バックアップセンターの「案件工房データ」カテゴリに含まれる。

- `shared/src/types/caseWorkshop.ts` — 案件・ステータス（11種）・要件整理・工程・成果物・品質チェック・納品パック・振り返りの型を一元定義。
- `src/orchestration/caseWorkshop.ts` + `routes/cases.ts` — 6種のAI処理を**各1回のツール強制呼び出し**で実装（`POST /api/cases/{requirements,questions,tasks,deliverable,quality,delivery-pack}`）。担当は領域に合うAI社員の人格を使う（要件整理=セイラ / 確認文・納品パック=ネムリ / 工程生成=レヴィ / 成果物=工程の担当AI / 品質チェック=ケイオス）。担当が削除されていても稼働中の社員へ自動フォールバック。要件整理は「依頼文にない事実は推測せず未確認とする」ルールをプロンプトで強制。工程生成は`minItems`＋空配列時エラーで確実に3件以上返す。
- `src/components/CaseWorkshop.tsx` — 一覧（絞り込み6種・検索・利益/承認バッジ、すべてクライアント処理）⇄詳細（基本情報・依頼文・要件整理〔手動編集可〕・クライアント確認文・作業工程・成果物〔承認/差し戻し/複製/保管庫へ保存/修正指示9種〕・品質チェック・納品パック・利益計算＋振り返り）のアコーディオン構成。全編集は`utils/cases.ts`経由で自動保存（保存中/保存済み表示）。利益計算（実利益・時給換算、固定/割合手数料）はAI不使用。
- AI呼び出し回数: 通常操作は0回（一覧・検索・利益計算・自動保存・承認）。要件整理・確認文・工程・品質・納品パックが各1回、成果物は工程ごとにユーザーが個別実行時のみ。同一操作の再実行時は確認ダイアログを表示。外部送信・自動投稿は一切なし。

#### 案件スカウター（募集案件の受注判定・MVP）

応募前の募集案件を貼り付けると「受注すべきか」を⭕（受注候補）/△（条件付き）/❌（見送り推奨）で判定する。案件工房の一覧から開き、履歴は`localStorage`（`chaos-ai-suite:scouts`）に保存されてバックアップセンターの「案件工房データ」カテゴリに含まれる。

- `shared/src/types/caseScout.ts` — 判定3段階・即納レベル5段階・応募/受注ステータス9種・分析結果（判定理由/強み/難点/リスク/専門警告/AI社員体制/工程案/応募文4種/確認質問3種）の型定義。
- `src/orchestration/caseScout.ts` + `POST /api/cases/scout` — 判定から応募文まで**全てを1回のツール強制呼び出し**で生成（担当はセイラ、削除時は稼働中社員へフォールバック）。プロンプトで「本文にない条件を推測で確定しない」「持っていない実績・資格を応募文に書かない（虚偽実績禁止）」「専門資格・法的リスク該当時は警告し判定を下げる」を強制。不正な判定値は△/追加確認後へ安全側フォールバックし、工程担当のagentIdは名簿と突合して検証する。
- `src/components/CaseScout.tsx` — 一覧（成功率カード〔応募率・返信率・受注率・受注合計、AI不使用の自動集計〕、判定/状態の絞り込み7種、並び替え4種）⇄詳細（案件情報入力→判定→結果表示→利益シミュレーション→応募文/質問文コピー→案件工房へ登録）。スクリーンショット解析は非対応で「スクリーンショット内の文章をコピーして貼り付けてください。」と案内し、個人情報を消してから貼るよう注意書きを表示。
- 「案件工房へ登録」はAI不使用のクライアント変換（工程案→作業工程、確認質問→クライアント確認文、判定サマリー・応募文→メモ）。`linkedCaseId`で二重登録を防止し、登録済みは「案件工房で開く」に変わる。
- 安全設計: 応募・送信・受注の自動化は一切なし（応募文はコピーして自分で送る）。再判定は確認ダイアログ、利益計算・集計・並び替えはすべてクライアント処理でAI呼び出し0回。

#### トレンドnote生成AI（Trend Research Note Generator・MVP）

投稿テーマを考えなくても、ボタン1つで最新トレンドを調査し、調査日・情報源・注目理由つきのnote記事とSNS告知文を生成する。履歴は`localStorage`（`chaos-ai-suite:trend-notes`・最新50件）に保存され、バックアップセンターの「note編集データ」カテゴリに含まれる。

- `shared/src/types/trendNote.ts` — ジャンル/読者/調査期間/記事形式/文字数/文体の選択肢、情報源（title/url/sourceName/publishedAt/retrievedAt/summary/platform/searchKeyword/信頼性）、テーマ評価、ファクトチェック5分類、生成履歴（状態5種）の型定義。
- `src/orchestration/llmClient.ts` — `callToolWithWebSearch`を追加。AnthropicのWeb検索サーバーツール（`web_search_20260209`・max_uses上限つき）を1回のAPI呼び出しに組み込み、`pause_turn`の継続にも対応。検索プロバイダーはLlmClient抽象の背後にあり将来交換できる。
- `src/orchestration/trendNote.ts` + `routes/trend.ts` — AI呼び出しは**2回に固定**: ①`POST /api/trend/research`（担当ミライ）＝Web検索最大5回→テーマ候補3件+情報源+注意事項を1回で生成。②`POST /api/trend/generate`（担当ネムリ）＝選択テーマから本文・ハッシュタグ・タイトル10案（狙い5タグ分散）・Threads/X/Instagram告知・ファクトチェック分類まで1回で生成（検索なし・調査結果を再利用）。
- 信頼性設計: 検索数など取得できない数値は推測せず「高・中・低」の相対評価。情報源にない事実の追加禁止・推測は「可能性があります」表現・断定には複数情報源・料金/仕様/法律は断定しない・記事コピー禁止をプロンプトで強制。記事冒頭に調査日を自動表記。ファクトチェックは確認済み/複数情報源あり/単一情報源/推測を含む/要確認の5分類（不明な分類は安全側の「要確認」へ）。情報源2件未満なら「信頼できる情報が不足しているため、記事生成を停止しました。」で停止。
- `src/components/TrendNoteStudio.tsx` — 設定→大きな生成ボタン→7ステップ進行表示→テーマ3件カード（相対評価バッジ）→タブ4枚（調査結果/note本文/SNS告知/タイトル案）。本文は編集可（userEditsとして自動保存）。全文/タイトル/本文/ハッシュタグ/Threads文の個別コピー＋「クリップボードにコピーしました」トースト。「要確認」が残る場合は投稿前警告。過去記事との重複は文字2-gram類似度（クライアント処理・AI不使用）で判定し、警告と代替案を表示。
- 安全設計: noteへの自動投稿・スクレイピング・自動スケジュール投稿は一切なし（コピーして自分で投稿）。生成前に確認ダイアログ（Web検索の使用を明示）。検索失敗時も画面はクラッシュせずエラーメッセージ表示。

#### Chaos Video Studio（テンプレート型・短尺MP4動画生成・MVP）

画像1〜5枚＋テロップからSNS投稿用の短いMP4動画を生成する。**生成は完全にブラウザ内で完結**し（Canvas描画＋端末内蔵エンコーダーWebCodecs＋`mp4-muxer`）、画像・動画はサーバーへ一切送信されない。AI呼び出しも0回。バックエンドの変更なし。

- `shared/src/types/videoStudio.ts` — VideoProject / VideoScene / VideoOutput型と上限（最大5枚・1枚10MB・1〜10秒/枚）、サイズ3種（9:16/1:1/16:9）、テンプレート3種（シンプル紹介/インパクト広告/note宣伝）、切替（フェード/カット）、演出（ゆっくりズーム/なし）。
- `src/utils/videoProjects.ts` — プロジェクト・画像Blob・完成動画のIndexedDB保存。既存BGM用DBには触れず独立DB（`chaos-ai-suite-video`）を使用。画像はJPEG/PNG/WebPのみ・サイズ上限・ファイル名無害化（セキュリティ要件）。
- `src/utils/videoRenderer.ts` — 描画・書き出しエンジン。プレビューと書き出しが同じ`drawFrame()`を共有。コーデックはH.264（互換性最優先）→VP9の順に自動選択し、非対応ブラウザには日本語エラー。エンコーダーの詰まり待ち＋UI小休止つきで30fps出力、進捗コールバックあり。
- `src/components/VideoStudio.tsx` — スマホ縦1列の7ステップUI: ①素材アップロード ②シーン編集（並び替え/テロップ/秒数/切替/演出）③テンプレート ④サイズ ⑤プレビュー（Canvasでリアルタイム再生）⑥生成（進捗バー）⑦完成動画（プレビュー+ダウンロード）。プロジェクトは自動保存されリロード後も復元。
- 安全設計: サーバーでコマンドを実行しない（任意コマンド実行の余地が構造的にない）/素材は端末内のみ（ユーザー間で混ざらない）/自動投稿なし。将来拡張（AI台本・BGM・立ち絵・字幕・外部動画API）は型とレンダラーの分離で追加しやすい構成。
