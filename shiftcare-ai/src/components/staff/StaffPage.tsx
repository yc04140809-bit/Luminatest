...
import { Plus, Moon, CloudMoon, Trash2, Pencil, Search, Users } from 'lucide-react';
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-4 gap-3">
        ...
        <Button icon={<Plus size={18} />} onClick={openNewStaffForm}>
          スタッフを追加
        </Button>
      </div>
      ...
      {staff.length === 0 ? (
        <EmptyState
          ...
          description="まずはスタッフを登録すると、シフト作成が始められます。"
          actionLabel="スタッフを追加"
          onAction={openNewStaffForm}
        />
      ) : (
        <>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            {filtered.length === 0 && (
              <div className="col-span-2 text-center text-slate-400 py-16">該当するスタッフがいません。</div>
            )}
          </div>
        </>
      )}
