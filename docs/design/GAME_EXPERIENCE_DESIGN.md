# CHAOS RE:BIRTH — Game Experience Design Ver.2.1
...
新機能・仕様変更・実装方針に迷ったとき、技術的な面白さ・機能の豪華さ・工数の少なさで判断してはならない。判断基準はただ一つ。

> **この機能でケイオスちゃんをもっと好きになるか?**

- **YES**(シンプルでも愛着が深まる) → 採用する。
- **NO**(豪華でも魅力を損なう、あるいは愛着と無関係) → 採用しない。機能として面白くても、ケイオスちゃんの魅力を薄めるなら却下する。

この基準は**第0.1節の3原則(時間は嘘をつかない/思い出は蓄積し劣化しない/UIは主張しない)よりも上位**に位置する。3原則はこの基準を満たすための手段であり、目的ではない。両者が矛盾する場面では常にこの基準を優先する。

**本ドキュメント以降のすべての章、第14章の追加提案20選、そして今後の実装判断のすべては、この基準によるフィルタリングを経ることを前提とする。** 機能を「増やす」ことそのものは価値を持たない。増える機能一つひとつが、この問いにYESで答えられるかどうかがすべてである。
