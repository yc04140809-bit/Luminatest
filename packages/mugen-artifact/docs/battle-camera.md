...
| `FOCUS` | 行動キャラが対象へ寄る | inset +0.08 | 0 | 0 |
| `RETREAT` | 他の味方が退避 | inset +0.08 | bottom +0.05 | 0 |
| `IMPACT` | 接触。最も踏み込む | inset +0.14 | bottom +0.05 | 0 |
| `RETURN` | 陣形へ帰る | 0 | 0 | 0 |
...
| | 量 | 844px幅での実測 |
|---|---|---|
| ACTOR の寄り（FOCUS） | 場の幅の 0.08 | **+68px** |
| ACTOR の踏み込み（IMPACT） | 場の幅の 0.14 | **+118px** |
| ALLY の退避（RETREAT） | 場の高さの 0.05 | **+11〜13px** |
...
**C-2a で実機調整済み**（初回値は 0.06 / 0.10 / 0.04 / glide 220）。

初回値の問題は移動量ではなく **glide が phase 間隔より長かった**ことでした。
220ms かけて移動する間に次のphaseが目標を書き換えるため、
FOCUS も IMPACT も**到達する前に次へ進み**、場が「歩いた」ではなく
「漂った」ように見えていました。glide 120ms でこれが解消しています。
...
- cueは全てその**割合**（0 / 0.34 / 0.72 / beat終 / turn終）
