...
import { beatMs, BATTLE_SPEEDS, type BattleSpeed } from '@mugen/game/battle/battleSpeed';
import {
  createBattle,
  castMagic,
  playerAttack,
...
import { decideTurn } from '@mugen/game/battle/magicChoice';
import { MAGIC_DEFS } from '@mugen/content/magic/magicDefs';
import { partyFormation, MAX_PARTY } from './formation';

/**
 * IT LIVES ON THIS SIDE OF THE LINE NOW, and the move is the point.
 *
 * It used to sit in `game/battle`, which is shared code, while reading
 * `ui/battle/formation`, which is not — so the shared core could not
 * be lifted out without dragging a screen behind it. The test itself
 * is unchanged: it still reads the battle's own source and still
 * checks the diamond. What changed is that the one file spanning both
 * sides is filed under the side that can see both.
 */
const here = join(__dirname, '../../../../mugen-core/game/battle');
const rngMid = () => 0.5;
