...
6. 【アーティファクト】新しい素材（横長の森画3.1MB、ガルドの倒れ絵2.3MB）で
   単一HTMLが**21.97MB**まで膨らみ、公開できませんでした。前ラウンドで
   ご承認いただいた**「アーティファクト用だけ再エンコード」の仕組みに
   4点追加**（森画・ガルド倒れ絵・モスラビット2点・ケイオスのシート）し、
   **9.47MB**で公開しました。**リポジトリの元PNGは1バイトも変更していません。
   解像度も変えていません**（ケイオスのシートは切り出し矩形が効くよう、
   特に同解像度であることが必要です）。実機の見え方・E2E・スクリーンショットは
   すべてリポジトリ側の元PNGが基準です。
7. 【緩めたガード】E2Eの**テスト制限時間を60秒→240秒**、
